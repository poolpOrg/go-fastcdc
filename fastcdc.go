...
func NewChunker(reader io.Reader, options *ChunkerOpts) (*Chunker, error) {
	if options != nil {
		if options.NormalSize == 0 || options.NormalSize < 64 || options.NormalSize > 1024*1024*1024 {
			return nil, errors.New("NormalSize is required and must be 64B <= NormalSize <= 1GB")
		}
		if options.MinSize == 0 {
			options.MinSize = options.NormalSize / 4
			if options.MinSize < 64 {
				options.MinSize = 64
			}
		}
		if options.MaxSize == 0 {
			options.MaxSize = options.NormalSize * 4
			if options.MaxSize > 1024*1024*1024 {
				options.MaxSize = 1024 * 1024 * 1024
			}
		}
		if options.MinSize < 64 || options.MinSize > 1024*1024*1024 || options.MinSize >= options.NormalSize {
			return nil, errors.New("MinSize is required and must be 64B <= MinSize <= 1GB && MinSize < NormalSize")
		}
		if options.MaxSize < 64 || options.MaxSize > 1024*1024*1024 || options.MaxSize <= options.NormalSize {
			return nil, errors.New("MaxSize is required and must be 64B <= MaxSize <= 1GB && MaxSize > NormalSize")
		}
	}
	...
	if options == nil {
		...
		chunker.MinSize = 2 * 1024
		chunker.MaxSize = 64 * 1024
	} else {
		chunker.NormalSize = options.NormalSize
		chunker.MinSize = options.MinSize
